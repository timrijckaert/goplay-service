...
internal object AuthenticationHelper {
    ...
    fun userSrpAuthRequest(
        ...
        try {
            ...
            val secretBlock: ByteArray = Base64.getDecoder().decode(challenge.challengeParameters()["SECRET_BLOCK"]!!)
        ...
        return RespondToAuthChallengeRequest.builder().challengeName(ChallengeNameType.PASSWORD_VERIFIER).clientId(COGNITO_CLIENT_ID)
            .session(challenge.session()).challengeResponses(
                mapOf(
                    "PASSWORD_CLAIM_SECRET_BLOCK" to challenge.challengeParameters()["SECRET_BLOCK"]!!, "PASSWORD_CLAIM_SIGNATURE" to String(
                        Base64.getEncoder().encode(hmac), Charsets.UTF_8
                    ), "TIMESTAMP" to formatTimestamp.format(timestamp), "USERNAME" to usernameInternal
                )
            ).build()
    }
    ...
    fun initiateUserSrpAuthRequest(username: String): InitiateAuthRequest? =
        InitiateAuthRequest.builder().authFlow(AuthFlowType.USER_SRP_AUTH).clientId(COGNITO_CLIENT_ID).authParameters(
            mutableMapOf(
                "USERNAME" to username, "SRP_A" to A.toString(16)
            )
        ).build()

    fun refreshToken(refreshToken: String): InitiateAuthRequest =
        InitiateAuthRequest.builder().authFlow(AuthFlowType.REFRESH_TOKEN_AUTH).authParameters(
            mapOf(
                "REFRESH_TOKEN" to refreshToken
            )
        ).clientId(COGNITO_CLIENT_ID).build()
}
