...
public class EpisodeParserTest : StringSpec({
    "should be able to parse `Big Brother`" {
        ...
        val episodeJson =
            javaClass.classLoader?.getResourceAsStream("big-brother-episode.json")!!.reader()
                .readText()
        val episodeParser = EpisodeParser()
        val episode: Either<ApiResponse.Failure, Program.Playlist.Episode> =
            episodeParser.parse(episodeJson)
        episode.shouldBeRight()
    }

    "should be able to parse `Stukken van mensen` episode" {
        val episodeJson =
            javaClass.classLoader?.getResourceAsStream("stukken-van-mensen-episode.json")!!.reader()
                .readText()
        val episodeParser = EpisodeParser()
