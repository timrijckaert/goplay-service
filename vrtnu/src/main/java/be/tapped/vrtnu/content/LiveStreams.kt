...
public object LiveStreams {
    ...
    public val ketnetJunior: LiveStream =
        LiveStream(
            name = "Ketnet Junior",
    ...
    public val mnm: LiveStream =
        LiveStream(
            name = "MNM",
            brand = LiveStream.Brand.MNM,
            videoId = VideoId("vualto_mnm")
        )

    public val vrtEvents1: LiveStream =
        LiveStream(
            name = "VRT Events 1",
            brand = LiveStream.Brand.VRT_NXT,
            videoId = VideoId("vualto_events1_geo")
        )

    public val vrtEvents2: LiveStream =
        LiveStream(
            name = "VRT Events 2",
            brand = LiveStream.Brand.VRT_NXT,
            videoId = VideoId("vualto_events2_geo")
        )

    public val vrtEvents3: LiveStream =
        LiveStream(
            name = "VRT Events 3",
            brand = LiveStream.Brand.VRT_NXT,
            videoId = VideoId("vualto_events3_geo")
        )
