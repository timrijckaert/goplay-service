...
public interface StreamRepo {
    ...
    public suspend fun getVODStream(
        vrtPlayerToken: VRTPlayerToken, videoId: VideoId, publicationId: String
...
public class HttpStreamRepo(
    ...
    private fun constructVideoStreamUrl(
        ...
        HttpUrl.Builder().scheme("https").host("media-services-public.vrt.be").addPathSegments("vualto-video-aggregator-web/rest/external/v1")
            .addPathSegment("videos").apply {
                val vId = videoId.id
                publicationId?.let {
                    addEncodedPathSegment("${publicationId}$${vId}")
                } ?: addEncodedPathSegment(vId)
            }.addQueryParameter("vrtPlayerToken", vrtPlayerToken.vrtPlayerToken).addQueryParameter("client", CLIENT).build()

    override suspend fun getLiveStream(
        vrtPlayerToken: VRTPlayerToken, videoId: VideoId
    ): Either<Failure, Success.Content.StreamInfo> = getStream(
        constructVideoStreamUrl(
            vrtPlayerToken, videoId, null
        )
    )
    ...
    override suspend fun getVODStream(
        ...
        constructVideoStreamUrl(
            vrtPlayerToken, videoId, publicationId
        )
    )

    private suspend fun getStream(httpUrl: HttpUrl): Either<Failure, Success.Content.StreamInfo> = withContext(Dispatchers.IO) {
        val videoStreamResponse = client.executeAsync(
            Request.Builder().get().url(httpUrl).build()
        )
